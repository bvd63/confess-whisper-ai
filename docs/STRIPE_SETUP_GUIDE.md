...
STRIPE_WEBHOOK_SECRET=whsec_...      # Webhook signing secret (Step 3)
STRIPE_PRICE_VIP_MONTHLY=price_...   # Monthly price ID from Step 1.1
STRIPE_PRICE_VIP_YEARLY=price_...    # Yearly price ID from Step 1.2
VITE_STRIPE_PRICE_VIP_MONTH_ID=price_...  # Same as monthly (for frontend)
VITE_STRIPE_PRICE_VIP_YEAR_ID=price_...   # Same as yearly (for frontend)
```
...
> ⚠️ **Note**: Backend secrets (STRIPE_SECRET_KEY, STRIPE_WEBHOOK_SECRET) are managed in Lovable Cloud only.
...
STRIPE_PRICE_VIP_MONTHLY=price_live_...    # Live monthly price
STRIPE_PRICE_VIP_YEARLY=price_live_...     # Live yearly price
...
Create a new webhook endpoint in **Live Mode** with the same configuration as Step 3.
...
- Environment: `STRIPE_PRICE_VIP_MONTHLY`, `STRIPE_PRICE_VIP_YEARLY`

> 💡 **Security Note**: Price IDs are **public identifiers** and safe to expose client-side. Secret keys must remain in backend only.
