...
No new variables are required, but make sure production uses the consolidated naming:

- `PRICE_VIP_MONTHLY` / `PRICE_VIP_YEARLY` - Active backend allowlist entries (required)
- `PRICE_PREMIUM_MONTHLY` / `PRICE_PREMIUM_YEARLY` - Optional legacy premium support (empty if unused)
- `VITE_STRIPE_PRICE_VIP_MONTH_ID` / `VITE_STRIPE_PRICE_VIP_YEAR_ID` - Frontend price IDs (must match backend)

Legacy `STRIPE_PRICE_*` keys are still ingested for backward compatibility during the migration window, but plan to remove them once all environments have updated secrets.
