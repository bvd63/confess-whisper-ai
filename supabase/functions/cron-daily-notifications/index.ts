...
const logStep = (step: string, details?: any) => {
  ...
  console.log(`[CRON-DAILY] ${step}${detailsStr}`);
};

serve(async (req) => {
  try {
    logStep("Daily notifications cron started");
      ...
      try {
        ...
        const pushResponse = await fetch(`${supabaseUrl}/functions/v1/send-push`, {
          ...
          body: JSON.stringify({
            ...
            body: "Take a moment to confess and grow today 🌱",
    ...
    logStep("Daily notifications completed", { successCount, errorCount });
