-- Fix search_path security warning for award_coins function
...
BEGIN
  -- Check for duplicate transaction using session_id in description
  IF EXISTS (
    SELECT 1 FROM coin_transactions 
    WHERE description LIKE '%Session: ' || p_session_id || '%'
  ) THEN
  ...
  -- Record transaction with session_id in description
